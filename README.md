...
```bash
health        # 体系自检（定时任务/软链/报告新鲜度）+ 待处理队列（例外/待复查/未接入）
health -v     # 展开明细
```

退出码 0 = 无异常，1 = 有需要关注的东西（可接 shell 提示或 CI）。
安装：跑一次 `./install.sh`（软链到 `~/.local/bin/health`）；未装时直接用 `./status.sh`。
...
**[`DESIGN.md`](DESIGN.md)** —— 这套「分层沉淀 + 自动防腐烂」引擎的完整设计：三层模型与不变量、三条流水线、决策层、**失败模式与教训**（都是实测得出的通用结论）。改这套代码前先读它的「关键不变量」。

## conventions：通用工作约定

`plugins/conventions` 把一组**跨项目通用的工作约定**（决策方式、改动纪律、提交规范、自动化优先、协作方式）沉淀为可分享的资产。

- **唯一事实源**：`plugins/conventions/CONVENTIONS.md`（改约定只改它）

### 两种交付方式（同一份内容，受众不同）

| | **自己用（本机）** | **给别人用（分发）** |
|---|---|---|
| 机制 | `~/.claude/rules/conventions.md` 软链到本文件 | 插件 `SessionStart` hook |
| 采纳 | 跑一次 `./install.sh` | `/plugin marketplace add <repo>` + `/plugin install conventions@ai-dev-kit` |
| 加载 | 启动时——**原生、声明式、无执行** | 启动时注入（跑一条命令） |

**关键区分**：`~/.claude/rules/` 是「让**这台机器**生效」的机制，不是「给别人」的格式；分发要走插件。

- **自己用不要装插件**：会与规则**重复加载**（同一内容进上下文两遍）。
- **为什么插件必须用 hook**：plugin **装不了 rules**（无 `rules` 字段），根目录 `CLAUDE.md` 也被官方**明确忽略**；plugin 提供上下文的机制只有 skill（按需触发）与 hook。所以「插件分发 + 每次生效」只有 hook 一条路。

> ⚠️ **hook 路径未实机验证**（从未在真实环境装过）。对外分发前先装一次实测。
> 内容本身通用（纯 markdown，可搬到任何平台），只有**交付方式**绑 Claude（见 `DESIGN.md` §三）。
...
> **⚠️ `失败` 列的语义边界（勿当质量指标读）**：它只测**机制级失败**——技能找不到、调用崩了。技能**跑起来了但干得烂**，`tool_result` 是成功的。故这一列**结构性地接近 0**（实测 47 次 Skill/Agent 调用全 `is_error=False`，且 join 已验证有效，是真读数、不是掩盖 bug）。真正的质量信号是「**用户推翻/重做**」，实测约 **3%**（1–2/47）——存在但太稀疏，且需要 judge 事后判（见 BACKLOG 流水线 B）。**把它当质量指标会得出"一切正常"的错觉**，那正是本工具反复在治的假阴性。
...
**⓪ 跑 `health`（最快）** —— 一条命令看全貌（自检 + 例外队列 + 待复查 + 未接入项数），见顶部「状态一览」。
...
1. **与工具自己的既定原则一致**——报告与快照都写在 gitignore 的 `health/report/` 下（「输出不过仓库」），决策日志没理由破例。
2. **它抑制的噪声是本机的**——触发来自本机转录；别的机器转录不同、报的东西也不同。
3. **写进共享仓库有真实的正确性问题**——两人各自对同一资产记录决策，共享的 append-only JSONL 需要 `merge=union`（当时确实为此配过，**现已随该方案一并移除**），而 union 合并后取"最后一条"时，两台机器的追加顺序**没有意义**（谁先写取决于时钟），"最新"是任意的。
4. **变更理由本就该写在那个变更的 commit 里**——人执行 `retire`/`promote` 时会提交到对应仓库；再往工具状态文件存一份 = 第二份表示。

补充：动作会**自我了结**——真把某个 skill 从插件退役后，它就不在扫描范围内，任何机器都不再报它。日志里那条只是「已决定、未执行」的中间态，只需本机可见。

内置与外部 marketplace 的资产**拒绝写入**：它们不出现在待裁决队列里，为其记录决策没有意义。
...
- `health/` 已落地（流水线 A）。巡检报告与快照写入 `health/report/`（不共享，派生自本机转录）；**决策日志全部存本机**（`~/.claude/health-decisions.jsonl`，工具不往任何仓库写）。覆盖已从「仅 Skill」扩到「Skill + Agent」，并补齐失败信号、快照、晋升候选检测与「裁决后退出队列」的闭环。
